...
def update_exec_times(task_dir, time):
    ...
    conn = sqlite3.connect("db_server.db")
    conn.execute(
        "UPDATE Users SET ExecTime = ExecTime +"
        + str(time)
        + " WHERE UserId ="
        + str(user_id)
    )
...
def job_status(sock):
    ...
    if len(running_tasks) == 0:
        ...
        for key in running_tasks:
            ...
            message += (
                "    "
                + str(running_tasks[key])
                + " IS RUNNING ON CLIENT #"
                + str(addr_to_number[key])
                + ".        EXECUTION TIME: "
                + str(run_time)
                + " SECONDS.\n"
            )
            ...
            # Some incoming message from a client
                ...
                if data:
                    ...
                    if data.decode().startswith("SUBMIT"):
                        ...
                        ret_msg = (
                            "CONFIRM_TASK" + str(task_count - 1) + data.decode()[7:]
                        )
                        ...
                        send_results(
                            data.decode()[4 : 8 + digits],
                            data.decode()[8 + digits :],
                            sock,
                        )
                        ...
                        if sock in control_clients.values():
                            print(
                                "CONTROL CLIENT", control_clients[sock], "HAS CONNECTED"
                            )
                        else:
                        ...
                        if len(client_no) == 0:
                            ...
                            client_no[sock] = next(
                                filterfalse(
                                    set(client_no.values()).__contains__, count(1)
                                )
                            )
                        statistics[sock] = (
                            "Client #"
                        ...
                        broadcast_data(
                            sock,
                            "\r" + "<" + str(sock.getpeername()) + "> " + data.decode(),
                        )
